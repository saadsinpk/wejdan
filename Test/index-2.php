...
if ($result_prod_sql->num_rows > 0) {
	while($row_prod_sql = $result_prod_sql->fetch_assoc()) {
		...
		$product_upload = "INSERT INTO `ec_products` (`name`, `status`, `sku`, `quantity`, `allow_checkout_when_out_of_stock`, `with_storehouse_management`, `is_featured`, `brand_id`, `is_variation`, `sale_type`, `price`, `stock_status`, `created_by_type`) VALUES ('".mysqli_real_escape_string($conn, $product['product_title'])."', 'published', NULL, 100, '0', '1', '0', '0', '0', '0', '".mysqli_real_escape_string($conn, $product['product_price'])."', 'in_stock', 'Botble\\ACL\\Models\\User')";
		echo $product_upload;
		echo "<br>";
		exit();
		// echo $row_prod_sql['product_url'];
		// echo "<br>";
		// h1.pr-new-br
		exit();
	}
